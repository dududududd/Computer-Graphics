...
void World::build(void)
{
	...
	Torus* torus_ptr1 = new Torus(Radius1, radius1);
	torus_ptr1->set_material(reflective_ptr1);
	/*-----------------------*/

	/*设置红色圆环的各项参数*/
	Reflective* reflective_ptr2 = new Reflective;
	reflective_ptr2->set_ka(0.1);
	reflective_ptr2->set_kd(0.4);
	reflective_ptr2->set_cd(RGBColor(1, 0, 0));   	 // red
	reflective_ptr2->set_ks(0.25);
	reflective_ptr2->set_exp(100.0);
	reflective_ptr2->set_kr(0.85);
	reflective_ptr2->set_cr(RGBColor(1.0, 0.75, 0.75));  // red

	double radius2 = 0.5;
	double Radius2 = 4.0;

	Torus* torus_ptr2 = new Torus(Radius2, radius2);
	torus_ptr2->set_material(reflective_ptr2);

	Instance* instance_ptr1 = new Instance;
	instance_ptr1->set_object(torus_ptr2);
	instance_ptr1->rotate_x(90.0);
	instance_ptr1->translate(5.0, 0.0, 0.0);
	/*----------------------*/

	Compound* torus = new Compound;

	for (int i = 0; i < 2; i++)
	{
		torus->add_object(torus_ptr1);
		torus->add_object(instance_ptr1);
	}

	/*对两个圆环体组合实施变换*/
	Instance* instance_ptr2 = new Instance(torus);
	instance_ptr2->rotate_x(0.0);
	instance_ptr2->translate(-2.5, 0.0, 0.0);
	...
	reflective_ptr3->set_cr(RGBColor(0.9, 0.9, 0.9));  // light green
	...
	rectangle_ptr1->set_material(reflective_ptr3);
	...
	reflective_ptr4->set_cr(RGBColor(0.9, 0.9, 0.9));  // light green
	...
	rectangle_ptr2->set_material(reflective_ptr4);
	...
	rectangle_ptr3->set_material(reflective_ptr5);
	...
	rectangle_ptr5->set_material(reflective_ptr6);
	...
	reflective_ptr7->set_kr(1);
	reflective_ptr7->set_cr(RGBColor(1, 1, 1));  // lemon
	...
	rectangle_ptr4->set_material(reflective_ptr7);
