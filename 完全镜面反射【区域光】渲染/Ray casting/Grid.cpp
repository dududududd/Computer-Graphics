...
Grid::Grid(Mesh * _meshptr)
	...
	mesh_ptr(_meshptr),reverse_normal(false)
{
...
void Grid::setup_cells(void)
{
	...
	float wx = p1.x - p0.x;//x方向网格范围
	float wy = p1.y - p0.y;//y方向网格范围
	float wz = p1.z - p0.z;//z方向网格范围
	float multiplier = 2.0;//大约是对象的8倍
	...
	//删除Compound::Objects，但不要删除objects
	//objects.erase(objects.begin(), objects.end());

	//关于单元格对象计数的统计代码可以放在这里
	//这对于找出有多少单元没有对象、一个对象等非常有用如果你不想用的话就把它注释掉
...
bool Grid::hit(const Ray & ray, double & tmin, ShadeRec & sr)const
{
	...
	if (bbox.inside(ray.o))//光源是否在栅格内部
	{
	...
	{
		Point3D p = ray.o + t0 * ray.d;//具有栅格包围盒的初始命中点
...
/*以下函数读取PLY格式的文件，并在存储数据的地方构造网格三角形
在网格对象中,它们只是一些小的包装函数，它们调用函数read_ply_file或read_uv_ply_file做实际读数
它们使用gregturk的PLY代码来读取PLY文件
*/
...
{
	this->reverse_normal = true;
}

//将材质存储在指定对象中,这是从Rosette和Archway类调用的，它们继承自Grid
void Grid::store_material(Material * material, const int index)
{
	objects[index]->set_material(material);
}
...
/*
这个函数的大部分是由Greg Turk编写的，是根据许可协议发布的
在PLY.h和PLY.c文件的开头,PLY.h文件包含在该文件的开头,它还有一些printf语句可以调试
我已经做了一些修改来构造网格三角形并将它们存储在网格中,mesh_ptr是网格的一个数据成员,对象是复合的数据成员
三角逯型可以是平的，也可以是光滑的,使用一个函数构造平面和平滑三角形可以节省大量重复代码,平面和平滑三角形的ply文件是相同的
*/
